...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	SetInternalTx(evm, caller.Address(), to.Address(), value, uint64(evm.depth), err != nil)
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	SetInternalTx(evm, caller.Address(), caller.Address(), big.NewInt(0), uint64(evm.depth), err != nil)
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	...
	if maxCodeSizeExceeded || (err != nil && (evm.chainRules.IsHomestead || err != ErrCodeStoreOutOfGas)) {
		SetInternalTx(evm, caller.Address(), address, value, uint64(evm.depth), true)
		evm.StateDB.RevertToSnapshot(snapshot)
		if err != ErrExecutionReverted {
			contract.UseGas(contract.Gas)
		}
	} else {
		SetInternalTx(evm, caller.Address(), address, value, uint64(evm.depth), false)
	}
